...
- **Admin Password Management**: Admin-only feature on Users page. Two actions per user: 1) Send password reset email (POST /api/admin/users/:userId/send-reset-email) generates a 24-hour reset token and emails the user a reset link. 2) Set new password directly (POST /api/admin/users/:userId/reset-password) hashes and stores a new password immediately. Both endpoints require isAdmin middleware. UI includes Send icon button, KeyRound icon button, and a dialog for entering new passwords with 8-char minimum validation.

## Recent Changes (March 2026)

### Alert Preferences System
- **Component-Level Alert Filtering**: Users can select specific vendor components to receive alerts for. When enabled, only incidents mentioning those components trigger notifications. Schema: `componentFilters` text array on `user_vendor_subscriptions`. UI: Checkbox list in AlertPreferencesDialog with Select All/Clear All.
- **Alert Lifecycle Preferences**: Per-vendor toggles for New Incidents, Updates, and Resolved notifications. Schema: `alertOnNew`, `alertOnUpdate`, `alertOnResolved` booleans on `user_vendor_subscriptions`. Enforced in both `dispatchIncidentNotification` and `dispatchLifecycleNotification` in `server/notificationDispatcher.ts`.
- **Maintenance Reminders**: Users can opt into advance reminders before scheduled maintenance via `maintenanceReminder` boolean and `maintenanceReminderMinutes` (15/30/60/120/240/1440). Scheduler in `server/maintenanceReminder.ts` runs on each sync cycle, checks upcoming maintenance, and sends email/SMS reminders. Deduplication via `maintenance_reminders_sent` table. Also honors `alertOnMaintenance` toggle.
- **AlertPreferencesDialog**: New component `client/src/components/alert-preferences-dialog.tsx` accessible via gear icon on monitored vendor cards and detail panel. Sections: Incident Lifecycle toggles, Maintenance toggles with reminder timing dropdown, Component Filtering with grouped checkboxes.
- **API Endpoints**: GET/PUT `/api/vendor-subscriptions/:vendorKey/preferences` for reading/updating per-vendor alert preferences with input validation.

### UI Improvements (User Feedback Round)
- **Dashboard Redesign**: Replaced request volume graph and system process status with user-centric view: "Services Needing Attention" card showing degraded vendors, active incidents list, monitored services grid with status dots. Notification setup banner shown when email/SMS not configured. "All Systems Operational" card when no issues.
- **Sidebar Streamlining**: Sections (MSP Tools, Integrations, Admin) now collapsible with chevron toggles. `defaultCollapsed` controls initial state. Scroll position preserved across page navigations via useRef. Bell icon added to desktop sidebar header and mobile top bar for quick notification settings access.
- **Incident Filters**: Added severity (Critical/Major/Minor) and status (Investigating/Identified/Monitoring/Update) filter pills on Incidents page with counts. "Clear filters" button when filters active. Incident cards decluttered: removed raw ID display, simplified status badges.
- **Notification Discoverability**: Setup banner on dashboard when notifications not configured. Bell icon in desktop sidebar header and mobile header linking to /settings. Notification status footer on dashboard showing active channels.
- **JSON Error Handling**: Fixed `queryClient.ts` `throwIfResNotOk` to parse JSON error responses and extract `.error`/`.message` fields instead of showing raw JSON. Vendor mutations (toggle, request, direct add) use safe try/catch for error body parsing.
- **External Links**: Vendor detail panel now shows prominent "View [Vendor] Status Page" link with label text instead of tiny icon.
- **UI Declutter**: Configuration section (parser, endpoint) hidden for non-admin users. Status page link removed from vendor cards (still visible in detail). Incident cards show simplified status/date info. Vendor cards no longer show "Last checked" timestamp to reduce noise.
- **Sidebar Consolidation (Round 2)**: Removed Webhooks and PSA Ticketing from sidebar nav (accessible via Integrations hub page). Intelligence and Settings sections default-collapsed. Reduced visible nav items for cleaner sidebar.
- **Status Page Links**: Vendor detail panel uses prominent styled button for "View Status Page" link. Incident external links consistently labeled "Status Page" with button styling across active and archived incidents.
- **First-Subscribe Notification Prompt**: When a user subscribes to their very first vendor, a follow-up toast suggests configuring email/SMS notifications in Settings.