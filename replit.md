...
- **Notification Subscription Fix**: Fixed bug in notificationDispatcher.ts where users who hadn't configured vendor/blockchain subscriptions (hasUserSetSubscriptions=false) would receive zero notifications. The subscription check now uses hasUserSetSubscriptions/hasUserSetBlockchainSubscriptions to default unconfigured users to receiving all notifications. Fix applied to dispatchIncidentNotification, dispatchLifecycleNotification, and blockchain notification paths.